...
db.serialize(() => {
  ...
  `);
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        db.run(`INSERT INTO daily_tvl (date, tvl_stroops) VALUES (?, ?)
       ON CONFLICT(date) DO UPDATE SET tvl_stroops=excluded.tvl_stroops`, [dateStr, tvlStroops]);
    ...
    mockAddresses.forEach(addr => {
        db.run(`INSERT INTO active_institutions (address, last_active) VALUES (?, ?)
       ON CONFLICT(address) DO UPDATE SET last_active=excluded.last_active`, [addr, new Date().toISOString()]);
    });
    ...
    for (let i = 0; i < 8; i++) {
        ...
        db.run(`INSERT INTO recent_deals (id, type, amount, time, status, zkProof, health_factor)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            dealId,
